...
namespace TaxCalculatorInterviewTests
{
    ...
    public class TaxCalculator : ITaxCalculator
    {
        ...
        private Dictionary<Commodity, Tuple<DateTime, double>> _customRates = new Dictionary<Commodity, Tuple<DateTime, double>>();

        private bool IsKeyDUplicated()
        {
            return _customRates.GroupBy(x => x.Value).Where(x => x.Count() > 1).Any();
        }
